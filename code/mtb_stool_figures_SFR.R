...
mdwk8ZT1<-mdwk8%>%filter(ATTRIBUTE_condition!="NA" & ATTRIBUTE_timepoint=="ZT1")
mtbw8ZT1<-mtb%>% dplyr::select(`row ID`, any_of(mdwk8ZT1$sample_name))
write.table(mtbw8ZT1,"mtb/stool/data_files/20231108_gnps_run/quantification_table/quantification_table-00000-clean-mash-Wk8-ZT1FAFT.txt",sep = "\t",row.names = FALSE, quote=FALSE)

mdwk8ZT13<-mdwk8%>%filter(ATTRIBUTE_condition!="NA" & ATTRIBUTE_timepoint=="ZT13")
mtbw8ZT13<-mtb%>% dplyr::select(`row ID`, any_of(mdwk8ZT13$sample_name))
write.table(mtbw8ZT13,"mtb/stool/data_files/20231108_gnps_run/quantification_table/quantification_table-00000-clean-mash-Wk8-ZT13FAFT.txt",sep = "\t",row.names = FALSE, quote=FALSE)
...
mdwk8FAFT<-mdwk8%>%filter(ATTRIBUTE_condition!="NA")
mtbw8FAFT<-mtb%>% dplyr::select(`row ID`, any_of(mdwk8FAFT$sample_name))
write.table(mtbw8FAFT,"mtb/stool/data_files/20231108_gnps_run/quantification_table/quantification_table-00000-clean-mash-Wk8-FAFT.txt",sep = "\t",row.names = FALSE, quote=FALSE)
...
mdwk12ZT1<-mdwk12%>%filter(ATTRIBUTE_condition!="NA" & ATTRIBUTE_timepoint=="ZT1")
mtbw12ZT1<-mtb%>% dplyr::select(`row ID`, any_of(mdwk12ZT1$sample_name))
write.table(mtbw12ZT1,"mtb/stool/data_files/20231108_gnps_run/quantification_table/quantification_table-00000-clean-mash-Wk12-ZT1FAFT.txt",sep = "\t",row.names = FALSE, quote=FALSE)

mdwk12ZT13<-mdwk12%>%filter(ATTRIBUTE_condition!="NA" & ATTRIBUTE_timepoint=="ZT13")
mtbw12ZT13<-mtb%>% dplyr::select(`row ID`, any_of(mdwk12ZT13$sample_name))
write.table(mtbw12ZT13,"mtb/stool/data_files/20231108_gnps_run/quantification_table/quantification_table-00000-clean-mash-Wk12-ZT13FAFT.txt",sep = "\t",row.names = FALSE, quote=FALSE)
...
mdwk12FAFT<-mdwk12%>%filter(ATTRIBUTE_condition!="NA")
...
birdman_dat<-function(dat,annot){
  df<-dat%>%
    ...
    left_join(.,annot, by="FeatureID")%>%
    #mutate(Name=paste(FeatureID,Compound_Name, `most specific class`,sep=" "))%>%
    mutate(Name=paste(FeatureID,Compound_Name,mtb_group,sep=" "))%>%
    #mutate(Name=ifelse(is.na(Compound_Name),paste(FeatureID, "m/z",mz,`most specific class`,sep=" "),Name))%>%
...
md<-fread("mtb/stool/NASH_stool_metabolomics_metadata_new.txt")%>%
  mutate(ATTRIBUTE_condition=ifelse(is.na(ATTRIBUTE_condition),"NA",ATTRIBUTE_condition),
         ...
         diet=ifelse(ATTRIBUTE_condition=="NA","NCD","HFD"),
...
diet_rpca <- ggplot(rpca, aes(x =diet, y = PC1))+
  ...
  scale_y_continuous(breaks=c(-0.10,-0.05,0.,0.05,0.10))+
  labs(x = NA, y = "PC1") +
  # theme_void()
  theme_bw()+
  theme(panel.grid.major = element_blank(),
...
final_plot <- plot_grid(plot_b,plot_c,nrow=2,rel_heights  = c(4, 1.25))

ggsave("mtb/stool/rpca_results/rpca_results_NASH_all/SFR24_0125_NASH_stoolmtb_RPCA_sepwk_addbxplt.pdf", plot=final_plot,height=4.7, width=5.7)
...
p<-rpca %>%
  ggplot(aes(x=PC1, y=PC2, color=ATTRIBUTE_condition, shape=phase)) +
...
#263 cred it cut off is 0
# class<-fread("mtb/stool/canopus_summary.tsv")%>%
#   separate(name, c(NA,"FeatureID"),sep="sirius_")%>%
#   mutate(FeatureID=as.integer(FeatureID))

# mz_info<-fread("mtb/stool/NASH_TRF_blank-subtract-quant.csv")%>%
#   dplyr::select(1:2)%>%
#   dplyr::rename(mz=row_mz, FeatureID=sample_name)%>%
#   mutate(mz=signif(mz, digits = 5))

mz_info<-fread('mtb/stool/data_files/20231108_gnps_run/DB_result/mz_annot_v2.txt')

# annotations<-fread("files_from_KF/STAM-TRF-ileum-cecum-16S-study-13785/metabolomics/stool/data/DB_result/0c09c8ccb2db4225a56161af9d1ef166.tsv")%>%
#   rename(FeatureID="#Scan#")%>%select(FeatureID,Compound_Name)%>%
#   mutate(FeatureID=as.integer(FeatureID),
#          Compound_Name=gsub("\\(predic.*","",Compound_Name),
#          Compound_Name=gsub('"',"",Compound_Name))%>%
#   full_join(.,class,by="FeatureID")%>%
#   right_join(.,mz_info,by="FeatureID")
#write.table(annotations,"mtb/stool/feature_annotations_clean.txt",sep = "\t",row.names = FALSE,quote=FALSE)
...
#247 cred
#FT<-fread("mtb/stool/birdman/filtered_featuretable_allzt_NASH_TRF_FT.beta_var.tsv")
FT<-fread("mtb/stool/birdman/quantification_table-00000-clean-mash-Wk8-FT.beta_var.tsv")
FT<-birdman_dat(FT,annotations)%>%
...
write.table(FT,"mtb/stool/birdman/birdman_justcred_wk8FT_nonNASHNASH_results.tsv",sep = "\t",row.names = FALSE, quote=FALSE) 
write.table(FT,"mtb/stool/birdman/birdman_justcred_wk8FT_nonNASHNASH_results_justannot.tsv",sep = "\t",row.names = FALSE, quote=FALSE)
...
#ggsave("mtb/stool/birdman/birdman_justcred_Wk8FT_nonNASHNASH.pdf",height=20, width=22)
#ggsave("mtb/stool/birdman/birdman_justcred_Wk8FT_nonNASHNASH_justannot.pdf",height=6, width=22)
#ggsave("mtb/stool/birdman/birdman_justcred_Wk8FT_nonNASHNASH_justannot_rmout.pdf",height=6, width=22)
ggsave("mtb/stool/birdman/birdman_justcred_Wk8FT_nonNASHNASH_cred0.5.pdf",height=6, width=8)
...
#   labs(title="FT Wk8\nnon-NASH vs. NASH")+
#   coord_flip()
# ggsave("16s/stool/birdman_results/birdman_justcred_Wk8FT_nonNASHNASH_colored.pdf",height=5, width=12)

#572 cred
#FT<-fread("mtb/stool/birdman/filtered_featuretable_allzt_NASH_NASH_FT.beta_var.tsv")
FT<-fread("mtb/stool/birdman/quantification_table-00000-clean-mash-Wk12-FT.beta_var.tsv")
FT<-birdman_dat(FT,annotations)%>%
  mutate(cat=ifelse(ratio<0,"Non_NASH","NASH"))%>%
  mutate(cat=factor(cat,levels=c("Non_NASH","NASH")))%>%
  filter(!grepl("m/z",Name))
write.table(FT,"mtb/stool/birdman/birdman_justcred_wk12FT_nonNASHNASH_results.tsv",sep = "\t",row.names = FALSE, quote=FALSE) 
write.table(FT,"mtb/stool/birdman/birdman_justcred_wk12FT_nonNASHNASH_results_justannot.tsv",sep = "\t",row.names = FALSE, quote=FALSE) 
write.table(FT,"mtb/stool/birdman/birdman_justcred_wk12FT_nonNASHNASH_results0.5cred.tsv",sep = "\t",row.names = FALSE, quote=FALSE)

birdman_plot(FT) + labs(title="FT Wk12\nnon-NASH vs. NASH")
ggsave("mtb/stool/birdman/birdman_justcred_Wk12FT_nonNASHNASH.pdf",height=26, width=22)
ggsave("mtb/stool/birdman/birdman_justcred_Wk12FT_nonNASHNASH_justannot.pdf",height=12, width=22)
ggsave("mtb/stool/birdman/birdman_justcred_Wk12FT_nonNASHNASH_cred0.5.pdf",height=12, width=12)
ggsave("mtb/stool/birdman/birdman_justcred_Wk12FT_nonNASHNASH_justannotcred0.5.pdf",height=6, width=12)

#168 cred
#FA<-fread("mtb/stool/birdman/filtered_featuretable_allzt_NASH_TRF_FA.beta_var.tsv")
FA<-fread("mtb/stool/birdman/quantification_table-00000-clean-mash-Wk8-FA.beta_var.tsv")
FA<-birdman_dat(FA,annotations)%>%
  mutate(cat=ifelse(ratio<0,"Non_NASH","NASH"))%>%
  mutate(cat=factor(cat,levels=c("Non_NASH","NASH")))%>%
  filter(!grepl("m/z",Name))
write.table(FA,"mtb/stool/birdman/birdman_justcred_wk8FA_nonNASHNASH_results.tsv",sep = "\t",row.names = FALSE, quote=FALSE) 
write.table(FA,"mtb/stool/birdman/birdman_justcred_wk8FA_nonNASHNASH_results_justannot.tsv",sep = "\t",row.names = FALSE, quote=FALSE)
...
ggsave("mtb/stool/birdman/birdman_justcred_Wk8FA_nonNASHNASH.pdf",height=20, width=21)
ggsave("mtb/stool/birdman/birdman_justcred_Wk8FA_nonNASHNASH_justannot.pdf",height=6, width=21)
ggsave("mtb/stool/birdman/birdman_justcred_Wk8FA_nonNASHNASH_cred0.5.pdf",height=5, width=12)

#508
#FA<-fread("mtb/stool/birdman/filtered_featuretable_allzt_NASH_NASH_FA.beta_var.tsv")
FA<-fread("mtb/stool/birdman/quantification_table-00000-clean-mash-Wk12-FA.beta_var.tsv")
FA<-birdman_dat(FA,annotations)%>%
  mutate(cat=ifelse(ratio<0,"Non_NASH","NASH"))%>%
  mutate(cat=factor(cat,levels=c("Non_NASH","NASH")))%>%
  filter(!grepl("m/z",Name))
write.table(FA,"mtb/stool/birdman/birdman_justcred_wk12FA_nonNASHNASH_results.tsv",sep = "\t",row.names = FALSE, quote=FALSE) 
write.table(FA,"mtb/stool/birdman/birdman_justcred_wk12FA_nonNASHNASH_results_justannot.tsv",sep = "\t",row.names = FALSE, quote=FALSE)
write.table(FA,"mtb/stool/birdman/birdman_justcred_wk12FA_nonNASHNASH_results0.5cred.tsv",sep = "\t",row.names = FALSE, quote=FALSE) 

birdman_plot(FA) + labs(title="FA Wk12\nnon-NASH vs. NASH")
ggsave("mtb/stool/birdman/birdman_justcred_Wk12FA_nonNASHNASH.pdf",height=25, width=21)
ggsave("mtb/stool/birdman/birdman_justcred_Wk12FA_nonNASHNASH_justannot.pdf",height=12, width=21)
ggsave("mtb/stool/birdman/birdman_justcred_Wk12FA_nonNASHNASH_cred0.5.pdf",height=20, width=13)
ggsave("mtb/stool/birdman/birdman_justcred_Wk12FA_nonNASHNASH_justannotcred0.5.pdf",height=6, width=13)


FAFT<-fread("mtb/stool/birdman/quantification_table-00000-c")
FA<-birdman_dat(FA,annotations)%>%
  mutate(cat=ifelse(ratio<0,"Non_NASH","NASH"))%>%
  mutate(cat=factor(cat,levels=c("Non_NASH","NASH")))%>%
  filter(!grepl("m/z",Name))
...
wk8FA<-fread("mtb/stool/birdman/birdman_justcred_wk8FA_nonNASHNASH_results.tsv")
wk12FA<-fread("mtb/stool/birdman/birdman_justcred_wk12FA_nonNASHNASH_results.tsv")
...
grid.draw(p)
pdf(file="mtb/stool/birdman/birdman_justcredoverlap_nonNASHNASH_FAWk8Wk12results.pdf")
grid.draw(p)
dev.off()
...
wk8FT<-fread("mtb/stool/birdman/birdman_justcred_wk8FT_nonNASHNASH_results.tsv")
wk12FT<-fread("mtb/stool/birdman/birdman_justcred_wk12FT_nonNASHNASH_results.tsv")
...
grid.draw(p)
pdf(file="mtb/stool/birdman/birdman_justcredoverlap_nonNASHNASH_FTWk8Wk12results.pdf")
grid.draw(p)
dev.off()
...
grid.draw(p)
pdf(file="mtb/stool/birdman/birdman_justcredoverlap_nonNASHNASH_wk8wk12_results.pdf")
grid.draw(p)
dev.off()
...
grid.draw(p)
pdf(file="mtb/stool/birdman/birdman_justcredoverlap_nonNASHNASH_wk8FAFTresults.pdf")
grid.draw(p)
dev.off()
...
grid.draw(p)
pdf(file="mtb/stool/birdman/birdman_justcredoverlap_nonNASHNASH_wk12FAFTresults.pdf")
grid.draw(p)
dev.off()
