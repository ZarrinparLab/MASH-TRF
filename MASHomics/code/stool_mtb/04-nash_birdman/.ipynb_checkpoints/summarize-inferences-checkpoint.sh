...
#SBATCH --chdir=/home/sfloresr/projects/NASH-KF/birdman_results/inferences/
#SBATCH --output=/home/sfloresr/projects/NASH-KF/birdman_results/slurm_out/%x.out
...
for inference_dir in glob.glob('/home/sfloresr/projects/NASH-KF/birdman_results/inferences/NASH_NASH*'):

    FEAT_REGEX = re.compile("F\d{4}_(.*).nc")
    omic_ = inference_dir.split('/')[-1]
    outfile = "inferences-results-NASH/%s.beta_var.tsv" % (omic_)
