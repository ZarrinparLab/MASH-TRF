...
#human only-oscillo
md_sub<-md%>%dplyr::select(sample_name,groups)
oscillo_abun <- m16s%>%filter(FeatureID %in% blast_oscillo$`subject acc.ver`)%>%
  gather(sample_name,abun, -FeatureID)%>%
  filter(abun>5)%>%arrange(desc(abun))%>%
  left_join(.,md_sub,by="sample_name")%>%
  mutate(MASH=ifelse(groups=="g3p",1,0))

plot(x=oscillo_abun$abun, oscillo_abun$MASH)

oscillo.glm.fit=glm(oscillo_abun$MASH ~ oscillo_abun$abun, family=binomial)
#lines(oscillo_abun$abun, oscillo.glm.fit$fitted.values)

#Area under the curve: 56.62%
roc(oscillo_abun$MASH, oscillo.glm.fit$fitted.values, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage")

#human only-lachno
md_sub<-md%>%dplyr::select(sample_name,groups)
lachno_abun <- m16s%>%filter(FeatureID %in% blast_lachno$`subject acc.ver`)%>%
  gather(sample_name,abun, -FeatureID)%>%
  filter(abun>5)%>%arrange(desc(abun))%>%
  left_join(.,md_sub,by="sample_name")%>%
  mutate(MASH=ifelse(groups=="g3p",1,0))

plot(x=lachno_abun$abun, lachno_abun$MASH)

lachno.glm.fit=glm(lachno_abun$MASH ~ lachno_abun$abun, family=binomial)
lines(lachno_abun$abun, lachno.glm.fit$fitted.values)

#Area under the curve: 54.77%
roc(lachno_abun$MASH, lachno.glm.fit$fitted.values, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage")

#samples with both lachno and oscillo

list_venn <- list(lachno = lachno_abun$sample_name,
                  oscillo = oscillo_abun$sample_name)

ItemsList <- venn(list_venn, show.plot = FALSE)
all<-attributes(ItemsList)$intersections

lachno_abun_sub<-m16s%>%filter(FeatureID %in% blast_lachno$`subject acc.ver`)%>%
  gather(sample_name,abun, -FeatureID)%>%
  arrange(desc(abun))%>%
  left_join(.,md_sub,by="sample_name")%>%
  mutate(MASH=ifelse(groups=="g3p",1,0))%>%
  filter(sample_name %in% all$`lachno:oscillo`)%>%
  group_by(sample_name,groups,MASH) %>%
  summarise(abun_comb=sum(abun))

oscillo_abun_sub<-m16s%>%filter(FeatureID %in% blast_oscillo$`subject acc.ver`)%>%
  gather(sample_name,abun, -FeatureID)%>%
  arrange(desc(abun))%>%
  left_join(.,md_sub,by="sample_name")%>%
  mutate(MASH=ifelse(groups=="g3p",1,0))%>%
...
comb_abun<-inner_join(lachno_abun_sub,oscillo_abun_sub, by="sample_name")

comb_abun_df<-comb_abun%>%column_to_rownames("sample_name")%>%dplyr::select("abun_comb.x","abun_comb.y")%>%
  as.data.frame()

# predict.glm(lachno.glm.fit, comb_abun[,"abun_comb.x"], type="response")
lachno.glm<-glm(MASH.x ~ abun_comb.x, family=binomial, data=comb_abun)
roc(comb_abun$MASH.x, lachno.glm$fitted.values, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage")

oscillo.glm<-glm(MASH.x ~ abun_comb.y, family=binomial, data=comb_abun)
roc(comb_abun$MASH.x, oscillo.glm$fitted.values, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage")

comb.glm<-glm(MASH.x ~ abun_comb.x+abun_comb.y, family=binomial, data=comb_abun)
roc(comb_abun$MASH.x, comb.glm$fitted.values, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage")

comb.glm<-glm(MASH.x ~ abun_comb.x*abun_comb.y, family=binomial, data=comb_abun)
roc(comb_abun$MASH.x, comb.glm$fitted.values, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage")

#mouse to human
mouse_md<-fread("16s/stool/metadata_cln_addmoreNASHcat.txt")%>%
...
mouse_m16s<-fread("files_from_KF/STAM-TRF-ileum-cecum-16S-study-13785/16S/stool/data/rrichter_preprocessed_20211020_ID_13785_gg2/NASH_taxonomy_filtered.gg2.asv.counts-rclr.txt")%>%
  dplyr::rename(FeatureID=`#OTU ID`)%>%
  gather(sample_name,counts,-FeatureID)%>%
  mutate(counts=ifelse(is.na(counts),0,counts))%>%
  left_join(.,mouse_md,by="sample_name")%>%
...
oscillo_mouse<-mouse_m16s%>%filter(FeatureID=="a6c1d5c393eaa8b78baddc67c010b5fe"& collection_disease_stage=="TRF")%>%
  mutate(MASH=ifelse(NASH_category=="NASH",1,0))
glm.fit=glm(oscillo_mouse$MASH ~ oscillo_mouse$counts, family=binomial)
roc(oscillo_mouse$MASH, glm.fit$fitted.values, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage")

oscillo_mouse_mash<-mouse_m16s%>%filter(FeatureID=="a6c1d5c393eaa8b78baddc67c010b5fe"& collection_disease_stage=="NASH")%>%
  mutate(MASH=ifelse(NASH_category=="NASH",1,0))
glm.fit=glm(oscillo_mouse_mash$MASH ~ oscillo_mouse_mash$counts, family=binomial)
roc(oscillo_mouse_mash$MASH, glm.fit$fitted.values, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage")

lachno_mouse<-mouse_m16s%>%filter(FeatureID=="e613626b2ce8bfbf28995d9b11edaeed"& collection_disease_stage=="TRF")%>%
  mutate(MASH=ifelse(NASH_category=="NASH",1,0))
glm.fit=glm(lachno_mouse$MASH ~ lachno_mouse$counts, family=binomial)
roc(lachno_mouse$MASH, glm.fit$fitted.values, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage")

lachno_mouse_mash<-mouse_m16s%>%filter(FeatureID=="e613626b2ce8bfbf28995d9b11edaeed"& collection_disease_stage=="NASH")%>%
  mutate(MASH=ifelse(NASH_category=="NASH",1,0))
glm.fit=glm(lachno_mouse_mash$MASH ~ lachno_mouse_mash$counts, family=binomial)
roc(lachno_mouse_mash$MASH, glm.fit$fitted.values, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage")
