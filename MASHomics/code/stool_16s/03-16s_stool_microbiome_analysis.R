...
##################################################################
#inputs
mash_metadata<-"~/Notebooks/sfloresr/MASH-TRF/MASHomics/data/mashstool16s_preprocessed_20211020_ID_13785_gg2/metadata.txt"
...
##################################################################
#Differential abundance analysis (BIRDMAn) results--Fig. S2B
##need to run BIRMAn scripts first
...
FT2<-fread("16s/stool/birdman_results/NASH_NASH_FT_taxonomy_filtered.gg2.asv.counts.beta_var.tsv")
FT2<-birdman_dat(FT2,annot)%>%
  mutate(cat=ifelse(ratio<0,"Non_NASH","NASH"))%>%
  mutate(cat=factor(cat,levels=c("Non_NASH","NASH")))%>%
  mutate(condition="FT", week=7)

FA2<-fread("16s/stool/birdman_results/NASH_NASH_FA_taxonomy_filtered.gg2.asv.counts.beta_var.tsv")
FA2<-birdman_dat(FA2,annot)%>%
  mutate(cat=ifelse(ratio<0,"Non_NASH","NASH"))%>%
  mutate(cat=factor(cat,levels=c("Non_NASH","NASH")))%>%
  mutate(condition="FA", week=7)

all_res<-rbind(FA,FT,FA2,FT2)
write.table(all_res,"16s/stool/birdman_results/birdman_nonNASHNASH_allresults_gg2.tsv",sep = "\t",row.names = FALSE, quote=FALSE) 
