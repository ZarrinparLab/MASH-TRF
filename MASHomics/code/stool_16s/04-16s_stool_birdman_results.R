...
bdm_res_w8_FA<-"~/Notebooks/sfloresr/MASH-TRF/MASHomics/results/stool_16s/birdman_results/NASH_TRF_FA_taxonomy_filtered.gg2.asv.counts.beta_var.tsv"
bdm_res_w8_FT<-"~/Notebooks/sfloresr/MASH-TRF/MASHomics/results/stool_16s/birdman_results/NASH_TRF_FT_taxonomy_filtered.gg2.asv.counts.beta_var.tsv"
bdm_res_w12_FA<-"~/Notebooks/sfloresr/MASH-TRF/MASHomics/results/stool_16s/birdman_results/NASH_NASH_FA_taxonomy_filtered.gg2.asv.counts.beta_var.tsv"
bdm_res_w12_FT<-"~/Notebooks/sfloresr/MASH-TRF/MASHomics/results/stool_16s/birdman_results/NASH_NASH_FT_taxonomy_filtered.gg2.asv.counts.beta_var.tsv"
...
bdm_dir<-"~/Notebooks/sfloresr/MASH-TRF/MASHomics/results/stool_16s/birdman_results/"
##################################################################
#functions
...
write.table(all_res,paste0(bdm_dir,"birdman_nonNASHNASH_allresults_gg2.tsv"),sep = "\t",row.names = FALSE, quote=FALSE) 
...
FA<-birdman_dat(FA,annot,"just_credible")%>%
  mutate(cat=ifelse(ratio<0,"Non_NASH","NASH"))%>%
  mutate(cat=factor(cat,levels=c("Non_NASH","NASH")))
write.table(FA,paste0(bdm_dir,"birdman_justcred_wk8FA_nonNASHNASH_results_gg2.tsv"),sep = "\t",row.names = FALSE, quote=FALSE) 

birdman_plot(FA) + labs(title="FA Wk8\nnon-NASH vs. NASH")
ggsave(paste0(bdm_dir,"birdman_justcred_Wk8FA_nonNASHNASH_gg2.pdf"),height=6, width=14)

##FA Wk7 (overall week 12), right
FA<-fread(bdm_res_w12_FA)
FA<-birdman_dat(FA,annot,"just_credible")%>%
  mutate(cat=ifelse(ratio<0,"Non_NASH","NASH"))%>%
  mutate(cat=factor(cat,levels=c("Non_NASH","NASH")))
write.table(FA,paste0(bdm_dir,"birdman_justcred_wk12FA_nonNASHNASH_results_gg2.tsv"),sep = "\t",row.names = FALSE, quote=FALSE) 

birdman_plot(FA) + labs(title="FA Wk12\nnon-NASH vs. NASH")
ggsave(paste0(bdm_dir,"birdman_justcred_Wk12FA_nonNASHNASH_gg2.pdf"),height=7, width=14)
...
FT<-birdman_dat(FT,annot,"just_credible")%>%
  mutate(cat=ifelse(ratio<0,"Non_NASH","NASH"))%>%
  mutate(cat=factor(cat,levels=c("Non_NASH","NASH")))
write.table(FT,paste0(bdm_dir,"birdman_justcred_wk8FT_nonNASHNASH_results_gg2.tsv"),sep = "\t",row.names = FALSE, quote=FALSE) 

birdman_plot(FT) + labs(title="FT Wk8\nnon-NASH vs. NASH")
ggsave(paste0(bdm_dir,"birdman_justcred_Wk8FT_nonNASHNASH_gg2.pdf"),height=6, width=13)

##FT Wk7 (overall week 12), right
FT<-fread(bdm_res_w12_FT)
FT<-birdman_dat(FT,annot,"just_credible")%>%
  mutate(cat=ifelse(ratio<0,"Non_NASH","NASH"))%>%
  mutate(cat=factor(cat,levels=c("Non_NASH","NASH")))
write.table(FT,paste0(bdm_dir,"birdman_justcred_wk12FT_nonNASHNASH_results_gg2.tsv"),sep = "\t",row.names = FALSE, quote=FALSE) 

birdman_plot(FT) + labs(title="FT Wk12\nnon-NASH vs. NASH")
ggsave(paste0(bdm_dir,"birdman_justcred_Wk12FT_nonNASHNASH_gg2.pdf"),height=4, width=13)

##################################################################
#Overlap of BIRDMAn results for NASH vs. non-NASH--Fig. 2A

##FA Wk4 to Wk7, left
wk8FA<-fread(paste0(bdm_dir,"birdman_justcred_wk8FA_nonNASHNASH_results_gg2.tsv"))
wk12FA<-fread(paste0(bdm_dir,"birdman_justcred_wk12FA_nonNASHNASH_results_gg2.tsv"))
...
grid.draw(p)
pdf(file=paste0(bdm_dir,"birdman_justcredoverlap_nonNASHNASH_FAWk8Wk12results_gg2.pdf"))
grid.draw(p)
dev.off()

##FT Wk8 to Wk12, right
wk8FT<-fread(paste0(bdm_dir,"birdman_justcred_wk8FT_nonNASHNASH_results_gg2.tsv"))
wk12FT<-fread(paste0(bdm_dir,"birdman_justcred_wk12FT_nonNASHNASH_results_gg2.tsv"))
...
grid.draw(p)
pdf(file=paste0(bdm_dir,"birdman_justcredoverlap_nonNASHNASH_FTWk8Wk12results_gg2.pdf"))
grid.draw(p)
dev.off()
...
FAw12<-fread(paste0(bdm_dir,"birdman_justcred_wk12FA_nonNASHNASH_results_gg2.tsv"))%>%
  arrange(desc(ratio))%>%rownames_to_column("rank")%>%mutate(week="Wk12",
                                                             Name_rank=paste(rank,Name, sep="."))

FAw8<-fread(paste0(bdm_dir,"birdman_justcred_wk8FA_nonNASHNASH_results_gg2.tsv"))%>%
...
ggsave(paste0(bdm_dir,"birdman_justcred_wk8wk12FA_alluvial_gg2.pdf"),height=5, width=8)

##FT data for non-nash vs nash going from wk 4 to wk 7, bottom
FTw12<-fread(paste0(bdm_dir,"birdman_justcred_wk12FT_nonNASHNASH_results_gg2.tsv"))%>%
  arrange(desc(ratio))%>%rownames_to_column("rank")%>%mutate(week="Wk12",
                                                             Name_rank=paste(rank,Name, sep="."))

FTw8<-fread(paste0(bdm_dir,"birdman_justcred_wk8FT_nonNASHNASH_results_gg2.tsv"))%>%
  arrange(desc(ratio))%>%rownames_to_column("rank")%>%mutate(week="Wk8",
                                                             Name_rank=paste(rank,Name, sep="."))
...
ggsave(paste0(bdm_dir,"birdman_justcred_wk8wk12FT_alluvial_gg2.pdf"),height=4, width=8)
...
ggsave(paste0(bdm_dir,"bdmFA7_65bc2_nonNASHNASH_gg2_unprd.pdf"),height=4 , width=4)

micro2<-paired_plots_ind(m16s,"7f782768e8d3067af0a923af092239d3","FA")
ggsave(paste0(bdm_dir,"bdmFA7_7f782_nonNASHNASH_gg2_unprd.pdf"),height=4 , width=4)

micro3<-paired_plots_ind(m16s,"726fe9527294f50f4d2654b3a25730a9","FA")
ggsave(paste0(bdm_dir,"bdmFA7_726fe_nonNASHNASH_gg2_unprd.pdf"),height=4 , width=4)

micro4<-paired_plots_ind(m16s,"eb4104bfe7ff965e7288b9e381317aa8","FA")
ggsave(paste0(bdm_dir,"bdmFA7_eb410_nonNASHNASH_gg2_unprd.pdf"),height=4 , width=4)

micro5<-paired_plots_ind(m16s,"d1c1529acbbd977e6219701a664424f0","FA")
ggsave(paste0(bdm_dir,"bdmFA7_d1c15_nonNASHNASH_gg2_unprd.pdf"),height=4 , width=4)

micro6<-paired_plots_ind(m16s,"e613626b2ce8bfbf28995d9b11edaeed","FA")
ggsave(paste0(bdm_dir,"bdmFA7_e6136_nonNASHNASH_gg2_unprd.pdf"),height=4 , width=4)

micro7<-paired_plots_ind(m16s,"d90a306bb67d75602d9e789894d2c81e","FA")
ggsave(paste0(bdm_dir,"bdmFA7_d90a3_nonNASHNASH_gg2_unprd.pdf"),height=4 , width=4)

##FT related hits
micro1<-paired_plots_ind(m16s,"d1c1529acbbd977e6219701a664424f0","FT")
ggsave(paste0(bdm_dir,"bdmFT4_d1c15_nonNASHNASH_gg2_unprd.pdf"),height=4 , width=4)

micro2<-paired_plots_ind(m16s,"a6c1d5c393eaa8b78baddc67c010b5fe","FT")
ggsave(paste0(bdm_dir,"bdmFT4_a6c1d_nonNASHNASH_gg2_unprd.pdf"),height=4 , width=4)

micro3<-paired_plots_ind(m16s,"be65daef90f69cfc35f5bcd8e387d377","FT")
ggsave(paste0(bdm_dir,"bdmFT4_be65d_nonNASHNASH_gg2_unprd.pdf"),height=4 , width=4)

micro4<-paired_plots_ind(m16s,"923a477c342aa445c4eecca0f8415ed5","FT")
ggsave(paste0(bdm_dir,"bdmFT4_923a4_nonNASHNASH_gg2_unprd.pdf"),height=4 , width=4)
